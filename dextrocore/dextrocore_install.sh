...
function prepare_node() { #TODO: add error detection
	...
	mkdir $TMP_FOLDER/installnode/dextro
	cp ~/rota/* $TMP_FOLDER/installnode/dextro
	#wget $COIN_TGZ_ZIP
        #$COIN_TAR_UNZIP
        rm *.gz >/dev/null 2>&1
        rm *.zip >/dev/null 2>&1
	   if [ ! -d "$TMP_FOLDER/installnode/$COIN_SUBFOLDER" ]; then cd $TMP_FOLDER/installnode/$COIN_SUBFOLDER ; fi
	   if [ $? -ne 0 ]; then end; fi
